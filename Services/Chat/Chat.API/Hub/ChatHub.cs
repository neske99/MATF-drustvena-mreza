  ...
  public class ChatHub : Hub
  {
    ...
    public async Task SendMessageToGroup(int chatGroupId, string message)
    {
      ...
      if (usr?.Identity?.IsAuthenticated == true)
      {
        ...
        await _chatService.CreateMessageForChatGroupAsync(userId, chatGroupId, message);

        await Clients.Group(chatGroupId.ToString()).SendAsync("ReceiveMessageReal", userId,message);
